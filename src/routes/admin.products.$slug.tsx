...
export const Route = createFileRoute("/admin/products/$slug")({
  loader: async ({ params }) => {
    ...
    return { product, originalSlug: params.slug };
  },
...
function EditProduct() {
  const { product, originalSlug } = Route.useLoaderData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminUpdateProduct({ data: { slug: originalSlug, product: form } });
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/95 backdrop-blur px-6 py-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-medium">{product.title.ru}</span>
        </div>
