...
import { adminGetJobs, adminUpdateJob } from "@/lib/api/admin.functions";
...
export const Route = createFileRoute("/admin/careers/$slug")({
  loader: async ({ params }) => {
    const jobs = await adminGetJobs();
    const job = jobs.find((j) => j.slug === params.slug);
    if (!job) throw new Error("Вакансия не найдена");
    return { job, originalSlug: params.slug };
  },
...
function EditCareer() {
  const { job, originalSlug } = Route.useLoaderData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await adminUpdateJob({ data: { slug: originalSlug, job: form } });
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-background/95 backdrop-blur px-6 py-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-medium">{job.title.ru}</span>
        </div>
